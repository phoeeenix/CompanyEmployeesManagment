...
public class List implements Comparable<List> {
    ...
    public float averageSalaryinDept(int number) {
    ...
    public float averageSalaryRatioFtoM() {
        float asF = 0;
        float asM = 0;
        float ratio;
        for (int i = 0; i < employeesList.size(); i++) {
            if (employeesList.get(i).getSex() == 'F') {
                asF += employeesList.get(i).getSalary();
            } else if (employeesList.get(i).getSex() == 'M') {
                asM += employeesList.get(i).getSalary();
            }
        }
        ratio = asF / asM;
        System.out.println("Average salary female to male ratio stand at: " + ratio);
        return ratio;
    }

    public float payRiseAll(float percentage) {
        float salaryBefore = 0;
        float salaryAfter = 0;
        float rise = 0;
        for (employee e : employeesList) {
            salaryBefore = e.getSalary();
            e.payRise(percentage);
            salaryAfter = e.getSalary();
            rise += salaryAfter - salaryBefore;
        }
        System.out.println("Salary for all employees has been raise at 10% + bonuses for having children and being married");
        System.out.println("Amount of all rises stand at: " + rise);
        return rise;
    }
